...
//XLS
    ...
    public static final void getXLSExportOfStackTrace(List<StackTrace> traces, boolean withFrames, Timestamp ts,File outputFile) throws IOException {
        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.createSheet("StackTraces");
        ...
        if (withFrames) {
            headerRow.createCell(3).setCellValue("Frames");
        }

        for (StackTrace trace : traces) {
            Row row = sheet.createRow(rowNum++);
            row.createCell(0).setCellValue(ts.getTime());
            row.createCell(1).setCellValue(trace.getException());
            row.createCell(2).setCellValue(trace.getReason());

            if (withFrames) {
                String frames = String.join("\n", trace.getFrames());
                row.createCell(3).setCellValue(frames);
            }
        }
    ...
    public static final void getXLSExportOfPatches(List<Patch> patches, boolean withHunks,File outputFile) throws IOException {
        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.createSheet("Patches");

        int rowNum = 0;

        Row headerRow = sheet.createRow(rowNum++);
        headerRow.createCell(0).setCellValue("Index");
        headerRow.createCell(1).setCellValue("Original File");
        headerRow.createCell(2).setCellValue("Modified File");

        if (withHunks) {
            headerRow.createCell(3).setCellValue("Hunks");
        }

        for (Patch patch : patches) {
            Row row = sheet.createRow(rowNum++);
            row.createCell(0).setCellValue(patch.getIndex());
            row.createCell(1).setCellValue(patch.getOriginalFile());
            row.createCell(2).setCellValue(patch.getModifiedFile());
            ...
            if (withHunks) {
                ...
                row.createCell(3).setCellValue(hunks);
            }
        }
    ...
    public static final void getXLSExportOfSourceCode(List<CodeRegion> coderegions, boolean withCode,File outputFile)throws IOException {
        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.createSheet("SourceCodeRegions");
        ...
        for (CodeRegion region : coderegions) {
            Row row = sheet.createRow(rowNum++);
            row.createCell(0).setCellValue(region.keyword);
            row.createCell(1).setCellValue(region.start);
            row.createCell(2).setCellValue(region.end);

            if (withCode) {
                row.createCell(3).setCellValue(region.text);
            }
        }
    ...
    public static final void getXLSExportOfEnumerations(List<Enumeration> enumerations, boolean withLines,File outputFile)throws IOException {
        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.createSheet("Enumerations");

        int rowNum = 0;

        Row headerRow = sheet.createRow(rowNum++);
        headerRow.createCell(0).setCellValue("Amount");
        headerRow.createCell(1).setCellValue("Enumeration");
        headerRow.createCell(2).setCellValue("Item");

        for (Enumeration enu : enumerations) {
            Row row = sheet.createRow(rowNum++);
            row.createCell(0).setCellValue(enumerations.size());
            row.createCell(1).setCellValue(enu.getEnumeration_items().size());

            if (withLines) {
                String lines = String.join("\n", enu.getEnumeration_items());
